...
import { Lock, Mail, Sparkles, UserCheck, Shield, KeyRound, Building2 } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (!name.trim() || !shopCode.trim()) {
          setErrorMsg('Veuillez renseigner votre nom et le Code Boutique.');
          setSubmitting(false);
          return;
        }
        ...
        if (data.success) {
          ...
          setErrorMsg(data.message || 'Erreur lors de l\'inscription employé.');
        }
      }
    } catch (err: any) {
    ...
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="auth-mode-segmented">
          ...
          <button
            type="button"
            className={`auth-mode-btn ${mode === 'employee' ? 'active' : ''}`}
            onClick={() => {
              setMode('employee');
              setErrorMsg(null);
              setSuccessMsg(null);
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          {mode === 'employee' && (
            <div className="auth-input-group">
              <label className="auth-label">
                <UserCheck className="w-3.5 h-3.5 inline mr-1 text-gold-deep" />
                Nom & Prénom Employé
              </label>
              <input
                type="text"
          ...
          {mode === 'employee' && (
            <div className="auth-input-group">
              <label className="auth-label">
                <KeyRound className="w-3.5 h-3.5 inline mr-1 text-gold-deep" />
                Code Boutique du Propriétaire
              </label>
              <input
                type="text"
                className="auth-input"
                placeholder="ex: SIFT-8820"
                value={shopCode}
          ...
          <button type="submit" className="auth-submit-btn" disabled={submitting}>
            {submitting
              ...
              : 'Créer mon Compte Employé & Soumettre'}
          </button>
        </form>
        ...
        <div className="auth-help-box">
          ...
          <p>
            {mode === 'employee' ? (
              <span><strong>Mode Employé :</strong> Votre compte sera transmis au propriétaire de la boutique <code>{shopCode || 'SIFT-XXXX'}</code> pour activation et attribution de vos accès.</span>
            ) : (
              <span><strong>Collaborateurs & Équipe :</strong> Utilisez l'onglet « Compte Employé » avec votre Code Boutique ou connectez-vous directement avec votre email d'accès.</span>
            )}
          </p>
        </div>
      </div>
